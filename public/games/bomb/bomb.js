...
function board(host=false){return `<div class="bombBoard">${(G.answers||[]).map((a,i)=>`<div class="bombTile ${a.revealed?'isRevealed':''} ${G.revealIndex===i&&G.phase==='reveal'?'aboutToReveal':''}">${a.revealed?`<div class="answerFace"><span class="answerNo">${i+1}</span><b>${esc(a.text)}</b><small class="${a.points<0?'negative':''}">${a.points>0?'+':''}${a.points} نقطة ${a.trap?'💣':''}</small></div>`:`<div class="questionFace"><span>?</span><small>إجابة ${i+1}</small></div>`}${(host||role==='player')&&!a.revealed&&G.phase!=='reveal'?`<button class="revealBtn" onclick="reveal(${i})">🔎 كشف الإجابة</button>`:''}</div>`).join('')}</div>`}
...
function answerInput(){return `<form class="typedPanel" id="answerForm" onsubmit="event.preventDefault();submit()"><div class="typedTitle">✍️ اكتب إجابتك</div><div class="inputRow"><input id="ans" name="answer" class="input answerInput" autocomplete="off" autocapitalize="sentences" autofocus placeholder="كلمة واحدة…"><button type="submit" class="sendAnswer">إرسال ↵</button></div><small>🧠 إذا كانت إجابتك موجودة في اللائحة تُحتسب تلقائيًا، حتى مع اختلاف بسيط في الكتابة.</small><div id="submitStatus" class="submitStatus" aria-live="polite"></div></form>`}
...
socket.on('joined',x=>{me=x.playerId;sessionToken=x.sessionToken||sessionToken;resumeBusy=false;saveBombSession();try{pendingAnswer=pendingAnswer||localStorage.getItem(bombSessionKey(code)+'.pending')||''}catch(e){}toastLocal('✓ تم الدخول للغرفة');socket.emit('room:sync',{code});render();if(pendingAnswer&&G?.phase==='answer'&&socket.connected){const v=pendingAnswer;setTimeout(()=>{socket.timeout(2200).emit('bomb:submit',{answer:v,sessionToken,playerId:me},(err,res)=>{if(!err&&res?.ok){pendingAnswer='';try{localStorage.removeItem(bombSessionKey(code)+'.pending')}catch(e){}}})},80)}});
socket.on('connect_error',()=>{toastLocal('تعذر الاتصال بالخادم. حاول تحديث الصفحة بعد ثوانٍ.');});
...
if(location.pathname.includes('/presenter')){role='presenter';socket.emit('room:presenter',{code})}else if(location.pathname.includes('/display')){role='display';socket.emit('room:display',{code})}else{role='player';const n=qp.get('name')||'لاعب',t=qp.get('team')==='B'?'B':'A',tok=qp.get('sessionToken')||'';if(tok){sessionToken=tok;socket.emit('room:resume',{code,sessionToken:tok,name:n,team:t})}else{
  // الدخول من كود الغرفة = لاعب جديد. لا نستعمل جلسة محلية قديمة تلقائيًا حتى لا يدخل اللاعب الثاني مكان لاعب آخر.
  socket.emit('room:join',{code,name:n,team:t});
}}
