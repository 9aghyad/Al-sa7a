...
function questionStage(){const listed=G.questionStats?.listed??(G.answers||[]).length,traps=G.questionStats?.traps??0,extra=G.questionStats?.extra??0;return `<div class="questionStage"><div class="questionPulseIcon">💣</div><div class="questionStageLabel">السؤال • تجهيز الجولة</div><h2>${esc(G.question||'')}</h2><div class="questionStats visibleStats"><span>📋 ${listed} إجابات في اللائحة</span><span>💣 ${traps} مفخخة مخفية</span><span>➕ ${extra} إجابات صحيحة خارج اللائحة</span></div><div class="stageCountdown">تبدأ أول إجابة بعد <b id="questionTimer">10</b> ثوانٍ</div></div>`}
...
function submit(){
  ...
  const send=()=>{
    ...
    socket.timeout(3000).emit('bomb:submit',{answer:v,sessionToken,playerId:me},(err,res)=>{
      if(!err&&res?.ok){
        pendingAnswer='';
      ...
      if(G?.phase==='answer'&&pendingAnswer===v&&attempts<12){
        ...
        setTimeout(send,220);
      }else if(status){status.textContent='لم يتم الإرسال بعد — لا تغلق الصفحة.';}
    });
  };
  send();
}
...
socket.on('resumeFailed',x=>{if(role!=='player'||!code)return;const saved=loadBombSession();const n=saved?.name||new URLSearchParams(location.search).get('name')||'لاعب';const t=saved?.team||new URLSearchParams(location.search).get('team')||'A';if(sessionToken&&!resumeBusy){resumeBusy=true;socket.emit('room:join',{code,name:n,team:t,sessionToken})}else if(!G){toastLocal('تعذر استعادة الغرفة. حاول الدخول بالكود من جديد.')}});
