...
function mafiaPublic(g,sid){
 ...
 myAction:g.phase==='night'?(g.nightActions?.[pid]||null):g.phase==='vote'?(g.votes?.[pid]?{submitted:true,targetId:g.votes[pid]}:null):null,
 isHost:host,winnerText:g.winnerText||null,chat:(g.chat||[]).slice(-60),voteResult:g.voteResult||null,
...
function scheduleBombTurn(g){}
 ...
 function mafiaNight(g){g.phase='night';g.phaseUntil=Date.now()+g.config.nightSec*1000;g.nightActions={};g.mafiaSelections={};g.votes={};g.investigationResults={};g.acks={};broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='night'&&g.phaseUntil===until)mafiaResolveNight(g)},g.config.nightSec*1000+100)}
 function mafiaResolveNight(g){const alive=g.players.filter(p=>p.alive!==false),actions=g.nightActions||{};const confirmed=alive.filter(p=>p.role==='mafia').map(p=>actions[p.id]?.submitted?actions[p.id].targetId:null).filter(Boolean);const victim=confirmed.length?g.players.find(p=>p.id===confirmed[0]&&p.alive!==false):null;const doctors=alive.filter(p=>p.role==='doctor').map(p=>actions[p.id]?.submitted?actions[p.id].targetId:null).filter(Boolean);if(victim&&!doctors.includes(victim.id)){victim.alive=false;g.lastNightResult={victimId:victim.id,victimName:victim.name,victimRole:g.config.revealRole?victim.role:undefined,saved:false}}else if(victim)g.lastNightResult={victimId:null,victimName:null,saved:true};else g.lastNightResult={victimId:null,victimName:null,saved:false};g.mafiaSelections={};if(mafiaCheckWin(g))return;g.phase='discussion';g.phaseUntil=Date.now()+g.config.discussionSec*1000;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='discussion'&&g.phaseUntil===until)mafiaStartVote(g)},g.config.discussionSec*1000+100)}
