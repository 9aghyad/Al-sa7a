...
function mafiaPublic(g,sid){
 ...
 const out={type:'mafia',code:g.code,phase:g.phase,phaseUntil:g.phaseUntil||0,
 players:g.players.map(p=>({id:p.id,name:p.name,alive:p.alive!==false,online:p.online!==false,status:p.online!==false?'online':'away',role:g.phase==='finished'?p.role:undefined})),
 config:g.config,myRole:me?.role,myAlive:me?.alive!==false,
 myAction:g.phase==='night'?(g.nightActions?.[pid]||null):g.phase==='vote'?(g.votes?.[pid]?{submitted:true,targetId:g.votes[pid]}:null):null,myVoteRequest:!!g.voteRequests?.[pid],
 isHost:host,winnerText:g.winnerText||null,earlyVoteCount:g.earlyVoteCount||0,earlyVoteTotal:g.earlyVoteTotal||0,revealVotes:!!g.config?.revealVotes,chat:(g.chat||[]).slice(-60),voteResult:g.voteResult||null,
 lastNightResult:g.phase==='discussion'?g.lastNightResult:null};
...
function scheduleBombTurn(g){}
 ...
 s.on('mafia:config',cfg=>{const g=games.get(s.data?.game);if(!g||g.type!=='mafia'||g.phase!=='lobby'||g.hostId!==s.data?.pid)return;const n=(v,d,min,max)=>Math.max(min,Math.min(max,Number(v)||d));const c={maxPlayers:n(cfg.maxPlayers,8,4,20),mafiaCount:n(cfg.mafiaCount,2,1,20),detectiveCount:n(cfg.detectiveCount,1,0,20),doctorCount:n(cfg.doctorCount,1,0,20),jesterCount:n(cfg.jesterCount,0,0,20),guardCount:n(cfg.guardCount,0,0,20),poisonerCount:n(cfg.poisonerCount,0,0,20),informantCount:n(cfg.informantCount,0,0,20),godfatherCount:n(cfg.godfatherCount,0,0,2),revealRole:!!cfg.revealRole,tieMode:cfg.tieMode==='revote'?'revote':'none',revealVotes:!!cfg.revealVotes,nightSec:n(cfg.nightSec,30,10,120),discussionSec:n(cfg.discussionSec,90,15,300),voteSec:n(cfg.voteSec,30,10,120)};if(c.mafiaCount+c.detectiveCount+c.doctorCount+c.jesterCount+c.guardCount+c.poisonerCount+c.informantCount+c.godfatherCount>c.maxPlayers)return s.emit('errorMsg','عدد الأدوار أكبر من عدد اللاعبين');if(g.players.length>c.maxPlayers)return s.emit('errorMsg','عدد اللاعبين الحالي أكبر من العدد المحدد');g.config=c;broadcastMafia(g)});
 s.on('mafia:start',()=>{const g=games.get(s.data?.game);if(!g||g.type!=='mafia'||g.phase!=='lobby'||g.hostId!==s.data?.pid)return;const c=g.config;if(g.players.length!==c.maxPlayers)return s.emit('errorMsg','يجب اكتمال عدد اللاعبين أولًا');const roleTotal=c.mafiaCount+c.detectiveCount+c.doctorCount+c.jesterCount+c.guardCount+c.poisonerCount+c.informantCount+c.godfatherCount;if(roleTotal>g.players.length)return s.emit('errorMsg','عدد الأدوار أكبر من عدد اللاعبين');let pool=[...g.players];for(let i=pool.length-1;i>0;i--){const j=crypto.randomInt(i+1);[pool[i],pool[j]]=[pool[j],pool[i]]}let k=0;pool.forEach(p=>{p.alive=true;p.role='citizen'});const assign=(role,count)=>{for(let i=0;i<count;i++)pool[k++].role=role};assign('mafia',c.mafiaCount);assign('godfather',c.godfatherCount);assign('detective',c.detectiveCount);assign('doctor',c.doctorCount);assign('guard',c.guardCount);assign('poisoner',c.poisonerCount);assign('informant',c.informantCount);assign('jester',c.jesterCount);g.rolesAssigned=true;g.phase='role';g.phaseUntil=Date.now()+15000;g.nightActions={};g.mafiaSelections={};g.votes={};g.investigationResults={};g.poisoned={};g.acks={};g.voteRequests={};g.voteResult=null;broadcastMafia(g);const roleUntil=g.phaseUntil;setTimeout(()=>{if(g.phase==='role'&&g.phaseUntil===roleUntil)mafiaNight(g)},15100)});
 s.on('mafia:ackRole',()=>{});
 s.on('mafia:requestVote',()=>{const g=games.get(s.data?.game),p=g?.players.find(x=>x.id===s.data?.pid);if(!g||g.type!=='mafia'||g.phase!=='discussion'||!p||p.alive===false||p.role==='jester')return;g.voteRequests=g.voteRequests||{};g.voteRequests[p.id]=true;const eligible=g.players.filter(x=>x.alive!==false&&x.role!=='jester');const count=Object.keys(g.voteRequests).filter(id=>eligible.some(x=>x.id===id)).length;g.earlyVoteCount=count;g.earlyVoteTotal=eligible.length;broadcastMafia(g);if(count>eligible.length/2)mafiaStartVote(g)});
 s.on('mafia:nightAction',({targetId,confirm=false}={})=>{const g=games.get(s.data?.game),p=g?.players.find(x=>x.id===s.data?.pid);if(!g||g.type!=='mafia'||g.phase!=='night'||!p||p.alive===false||Date.now()>g.phaseUntil)return;if(['citizen','jester'].includes(p.role))return;const target=g.players.find(x=>x.id===targetId&&x.alive!==false&&x.id!==p.id);if(!target)return;g.nightActions=g.nightActions||{};if(g.nightActions[p.id]?.submitted)return;if(['mafia','godfather'].includes(p.role)){g.mafiaSelections=g.mafiaSelections||{};g.mafiaSelections[p.id]=target.id;if(confirm)g.nightActions[p.id]={submitted:true,targetId:target.id};}else if(confirm){g.nightActions[p.id]={submitted:true,targetId:target.id};if(['detective','informant'].includes(p.role))g.investigationResults[p.id]=['mafia','godfather'].includes(target.role)?'mafia':'not_mafia';else if(p.role==='poisoner'){g.poisoned=g.poisoned||{};g.poisoned[target.id]={by:p.id,round:g.round||0};} }else g.nightActions[p.id]={submitted:false,targetId:target.id};broadcastMafia(g)});
s.on('mafia:chat',({text}={})=>{const g=games.get(s.data?.game),p=g?.players.find(x=>x.id===s.data?.pid);if(!g||g.type!=='mafia'||g.phase!=='discussion'||!p||p.alive===false)return;const t=String(text||'').trim().slice(0,180);if(!t)return;g.chat=g.chat||[];g.chat.push({id:crypto.randomBytes(5).toString('hex'),playerId:p.id,name:p.name,text:t,at:Date.now()});if(g.chat.length>100)g.chat=g.chat.slice(-100);broadcastMafia(g)});
 s.on('mafia:vote',({targetId}={})=>{const g=games.get(s.data?.game),p=g?.players.find(x=>x.id===s.data?.pid);if(!g||g.type!=='mafia'||g.phase!=='vote'||!p||p.alive===false||p.role==='jester'||g.votes[p.id])return;const target=g.players.find(x=>x.id===targetId&&x.alive!==false&&x.id!==p.id);if(!target)return;g.votes[p.id]=target.id;if(g.players.filter(x=>x.alive!==false&&x.role!=='jester').every(x=>g.votes[x.id]))mafiaResolveVote(g);broadcastMafia(g)});
 s.on('mafia:restart',()=>{const g=games.get(s.data?.game);if(!g||g.type!=='mafia'||g.hostId!==s.data?.pid||g.phase!=='finished')return;g.players.forEach(p=>{p.alive=true;p.role=undefined});g.rolesAssigned=false;g.phase='lobby';g.phaseUntil=0;g.nightActions={};g.mafiaSelections={};g.votes={};g.voteRequests={};g.earlyVoteCount=0;g.earlyVoteTotal=0;g.investigationResults={};g.poisoned={};g.acks={};g.lastNightResult=null;g.winnerText=null;broadcastMafia(g)});
 function mafiaNight(g){g.round=(g.round||0)+1;g.phase='night';g.phaseUntil=Date.now()+g.config.nightSec*1000;g.nightActions={};g.mafiaSelections={};g.votes={};g.voteRequests={};g.earlyVoteCount=0;g.earlyVoteTotal=0;g.investigationResults={};g.acks={};broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='night'&&g.phaseUntil===until)mafiaResolveNight(g)},g.config.nightSec*1000+100)}
 function mafiaResolveNight(g){const alive=g.players.filter(p=>p.alive!==false),actions=g.nightActions||{};const mafiaPlayers=alive.filter(p=>['mafia','godfather'].includes(p.role));const picks=mafiaPlayers.map(p=>actions[p.id]?.submitted?actions[p.id].targetId:null).filter(Boolean);const tally={};picks.forEach(id=>tally[id]=(tally[id]||0)+1);const max=Math.max(0,...Object.values(tally));const top=Object.keys(tally).filter(id=>tally[id]===max);const victim=top.length===1&&max>0?g.players.find(p=>p.id===top[0]&&p.alive!==false):null;const protects=alive.filter(p=>['doctor','guard'].includes(p.role)).map(p=>actions[p.id]?.submitted?actions[p.id].targetId:null).filter(Boolean);let saved=false;if(victim&&protects.includes(victim.id))saved=true;if(victim&&!saved){victim.alive=false;g.lastNightResult={victimId:victim.id,victimName:victim.name,saved:false}}else if(victim&&saved){g.lastNightResult={victimId:null,victimName:null,saved:true}}else{g.lastNightResult={victimId:null,victimName:null,saved:false}};const poisonTargets=Object.entries(g.poisoned||{}).filter(([id,v])=>{const p=g.players.find(x=>x.id===id);return p&&p.alive!==false&&Number(v?.round)<(g.round||0)}).map(([id])=>id);for(const id of poisonTargets){const p=g.players.find(x=>x.id===id);if(p&&!protects.includes(id)){p.alive=false;g.lastNightResult=g.lastNightResult?.victimId?g.lastNightResult:{victimId:p.id,victimName:p.name,saved:false,poisoned:true}}}g.mafiaSelections={};if(mafiaCheckWin(g))return;g.phase='discussion';g.phaseUntil=Date.now()+g.config.discussionSec*1000;g.voteRequests={};g.earlyVoteCount=0;g.earlyVoteTotal=alive.filter(p=>p.alive!==false&&p.role!=='jester').length;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='discussion'&&g.phaseUntil===until)mafiaStartVote(g)},g.config.discussionSec*1000+100)}
 ...
 function mafiaResolveVote(g){if(g.phase!=='vote')return;const counts={};Object.values(g.votes||{}).forEach(id=>counts[id]=(counts[id]||0)+1);const max=Math.max(0,...Object.values(counts));const top=Object.keys(counts).filter(id=>counts[id]===max);const rows=g.players.filter(p=>p.alive!==false).map(p=>({name:p.name,id:p.id,count:counts[p.id]||0})).sort((a,b)=>b.count-a.count);g.voteResult={rows,tie:!max||top.length!==1,eliminatedId:null,eliminatedName:null,eliminatedRole:null};if(!max||top.length!==1){g.phase='voteResult';g.phaseUntil=Date.now()+5600;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='voteResult'&&g.phaseUntil===until){if(g.config.tieMode==='revote'){g.voteResult=null;mafiaStartVote(g)}else{g.voteResult=null;mafiaNight(g)}}},5700);return}const victim=g.players.find(p=>p.id===top[0]&&p.alive!==false);if(victim){victim.alive=false;if(g.poisoned)delete g.poisoned[victim.id];g.voteResult.eliminatedId=victim.id;g.voteResult.eliminatedName=victim.name;g.voteResult.eliminatedRole=g.config.revealRole?victim.role:undefined;if(victim.role==='jester'){g.winnerText='🤡 المهرج فاز!';g.phase='voteResult';g.phaseUntil=Date.now()+5600;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='voteResult'&&g.phaseUntil===until){g.voteResult=null;g.phase='finished';g.finishedAt=Date.now();broadcastMafia(g)}},5700);return}}if(mafiaCheckWin(g))return;g.phase='voteResult';g.phaseUntil=Date.now()+5600;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='voteResult'&&g.phaseUntil===until){g.voteResult=null;mafiaNight(g)}},5700)}
 function mafiaCheckWin(g){const alive=g.players.filter(p=>p.alive!==false),m=alive.filter(p=>['mafia','godfather'].includes(p.role)).length,other=alive.length-m;if(m===0){g.phase='finished';g.winnerText='👥 المواطنون فازوا!';g.finishedAt=Date.now();broadcastMafia(g);return true}if(m>=other){g.phase='finished';g.winnerText='🔪 المافيا فازت!';g.finishedAt=Date.now();broadcastMafia(g);return true}return false}
