...
function mafiaPublic(g,sid){
 ...
 if(g.phase==='night'&&me?.role==='detective'&&g.investigationResults?.[pid]!==undefined)out.investigationResult=g.investigationResults[pid];
 if(g.phase==='night'&&me?.role==='mafia')out.mafiaSelections=Object.entries(g.mafiaSelections||{}).map(([id,targetId])=>({playerId:id,playerName:g.players.find(p=>p.id===id)?.name,targetId,targetName:g.players.find(p=>p.id===targetId)?.name}));
 return out;
}
...
function mafiaRoom(){return {type:'mafia',code:makeCode('mafia'),mode:'players',players:[],hostId:null,phase:'lobby',config:{maxPlayers:8,mafiaCount:2,detectiveCount:1,doctorCount:1,jesterCount:0,revealRole:true,revealVotes:false,tieMode:'none',nightSec:30,discussionSec:90,voteSec:30},rolesAssigned:false,acks:{},nightActions:{},mafiaSelections:{},votes:{},voteRequests:{},investigationResults:{},lastNightResult:null,winnerText:null,round:0,phaseUntil:0,chat:[]}}
...
function scheduleBombTurn(g){}
 ...
 s.on('mafia:nightAction',({targetId,confirm=false}={})=>{const g=games.get(s.data?.game),p=g?.players.find(x=>x.id===s.data?.pid);if(!g||g.type!=='mafia'||g.phase!=='night'||!p||p.alive===false||Date.now()>g.phaseUntil)return;if(p.role==='citizen'||p.role==='jester')return;const target=g.players.find(x=>x.id===targetId&&x.alive!==false&&x.id!==p.id);if(!target)return;g.nightActions=g.nightActions||{};if(g.nightActions[p.id]?.submitted)return;if(p.role==='mafia'){g.mafiaSelections=g.mafiaSelections||{};g.mafiaSelections[p.id]=target.id;if(confirm)g.nightActions[p.id]={submitted:true,targetId:target.id};}else if(confirm){g.nightActions[p.id]={submitted:true,targetId:target.id};if(p.role==='detective')g.investigationResults[p.id]=target.role;}else g.nightActions[p.id]={submitted:false,targetId:target.id};broadcastMafia(g)});
 ...
 function mafiaNight(g){g.phase='night';g.phaseUntil=Date.now()+g.config.nightSec*1000;g.nightActions={};g.mafiaSelections={};g.votes={};g.voteRequests={};g.earlyVoteCount=0;g.earlyVoteTotal=0;g.investigationResults={};g.acks={};broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='night'&&g.phaseUntil===until)mafiaResolveNight(g)},g.config.nightSec*1000+100)}
 function mafiaResolveNight(g){const alive=g.players.filter(p=>p.alive!==false),actions=g.nightActions||{};const confirmed=alive.filter(p=>p.role==='mafia').map(p=>actions[p.id]?.submitted?actions[p.id].targetId:null).filter(Boolean);const victim=confirmed.length?g.players.find(p=>p.id===confirmed[0]&&p.alive!==false):null;const doctors=alive.filter(p=>p.role==='doctor').map(p=>actions[p.id]?.submitted?actions[p.id].targetId:null).filter(Boolean);if(victim&&!doctors.includes(victim.id)){victim.alive=false;g.lastNightResult={victimId:victim.id,victimName:victim.name,victimRole:g.config.revealRole?victim.role:undefined,saved:false}}else if(victim)g.lastNightResult={victimId:null,victimName:null,saved:true};else g.lastNightResult={victimId:null,victimName:null,saved:false};g.mafiaSelections={};if(mafiaCheckWin(g))return;g.phase='discussion';g.phaseUntil=Date.now()+g.config.discussionSec*1000;g.voteRequests={};g.earlyVoteCount=0;g.earlyVoteTotal=alive.filter(p=>p.role!=='jester').length;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='discussion'&&g.phaseUntil===until)mafiaStartVote(g)},g.config.discussionSec*1000+100)}
 ...
 function mafiaResolveVote(g){if(g.phase!=='vote')return;const counts={};Object.values(g.votes||{}).forEach(id=>counts[id]=(counts[id]||0)+1);const max=Math.max(0,...Object.values(counts));const top=Object.keys(counts).filter(id=>counts[id]===max);const rows=g.players.filter(p=>p.alive!==false).map(p=>({name:p.name,id:p.id,count:counts[p.id]||0})).sort((a,b)=>b.count-a.count);g.voteResult={rows,tie:!max||top.length!==1,eliminatedId:null,eliminatedName:null,eliminatedRole:null};if(!max||top.length!==1){g.phase='voteResult';g.phaseUntil=Date.now()+5600;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='voteResult'&&g.phaseUntil===until){if(g.config.tieMode==='revote'){g.voteResult=null;mafiaStartVote(g)}else{g.voteResult=null;mafiaNight(g)}}},5700);return}const victim=g.players.find(p=>p.id===top[0]&&p.alive!==false);if(victim){victim.alive=false;g.voteResult.eliminatedId=victim.id;g.voteResult.eliminatedName=victim.name;g.voteResult.eliminatedRole=g.config.revealRole?victim.role:undefined;if(victim.role==='jester'){g.winnerText='🤡 المهرج فاز!';g.phase='voteResult';g.phaseUntil=Date.now()+5600;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='voteResult'&&g.phaseUntil===until){g.voteResult=null;g.phase='finished';g.finishedAt=Date.now();broadcastMafia(g)}},5700);return}}if(mafiaCheckWin(g))return;g.phase='voteResult';g.phaseUntil=Date.now()+5600;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='voteResult'&&g.phaseUntil===until){g.voteResult=null;mafiaNight(g)}},5700)}
