...
function mafiaPublic(g,sid){
 const sock=io.sockets.sockets.get(sid),pid=sock?.data?.pid,host=g.hostId===pid,me=g.players.find(p=>p.id===pid);
 const out={type:'mafia',code:g.code,phase:g.phase,phaseUntil:g.phaseUntil||0,round:g.round||0,players:g.players.map(p=>({id:p.id,name:p.name,alive:p.alive!==false,online:p.online!==false,status:p.online!==false?'online':'away',role:g.phase==='finished'?p.role:undefined})),config:g.config,myRole:me?.role,myAlive:me?.alive!==false,myPlayerId:pid,isHost:host,myAction:g.phase==='night'?(g.nightActions?.[pid]||null):g.phase==='vote'?(g.votes?.[pid]?{submitted:true,targetId:g.votes[pid]}:null):null,myVoteRequest:!!g.voteRequests?.[pid],winnerText:g.winnerText||null,earlyVoteCount:g.earlyVoteCount||0,earlyVoteTotal:g.earlyVoteTotal||0,revealVotes:!!g.config?.revealVotes,chat:(g.chat||[]).slice(-60),voteResult:g.voteResult||null,lastNightResult:g.phase==='discussion'?g.lastNightResult:null,mafiaConsensus:g.mafiaConsensus||null};
 if(g.phase==='night'&&['detective','informant'].includes(me?.role)&&g.investigationResults?.[pid]!==undefined)out.investigationResult=g.investigationResults[pid];
 if(g.phase==='night'&&['mafia','godfather'].includes(me?.role))out.mafiaSelections=Object.entries(g.mafiaSelections||{}).map(([id,targetId])=>({playerId:id,playerName:g.players.find(p=>p.id===id)?.name,targetId,targetName:g.players.find(p=>p.id===targetId)?.name,submitted:!!g.nightActions?.[id]?.submitted}));
 return out;
}
...
function mafiaRoom(){return {type:'mafia',code:makeCode('mafia'),mode:'players',players:[],hostId:null,phase:'lobby',config:{maxPlayers:8,mafiaCount:2,detectiveCount:1,doctorCount:1,jesterCount:0,guardCount:0,poisonerCount:0,informantCount:0,godfatherCount:0,revealRole:true,revealVotes:false,tieMode:'none',nightSec:30,discussionSec:90,voteSec:30},rolesAssigned:false,nightActions:{},mafiaSelections:{},votes:{},voteRequests:{},investigationResults:{},poisoned:{},lastNightResult:null,winnerText:null,round:0,phaseUntil:0,chat:[],voteResult:null,mafiaConsensus:null}}
...
function scheduleBombTurn(g){}
 ...
 s.on('mafia:config',cfg=>{
  const g=games.get(s.data?.game);if(!g||g.type!=='mafia'||g.phase!=='lobby'||g.hostId!==s.data?.pid)return;
  const n=(v,d,min,max)=>Math.max(min,Math.min(max,Number.isFinite(Number(v))?Number(v):d));
  const c={maxPlayers:n(cfg.maxPlayers,8,4,20),mafiaCount:n(cfg.mafiaCount,2,1,20),detectiveCount:n(cfg.detectiveCount,1,0,20),doctorCount:n(cfg.doctorCount,1,0,20),jesterCount:n(cfg.jesterCount,0,0,20),guardCount:n(cfg.guardCount,0,0,20),poisonerCount:n(cfg.poisonerCount,0,0,20),informantCount:n(cfg.informantCount,0,0,20),godfatherCount:n(cfg.godfatherCount,0,0,1),revealRole:!!cfg.revealRole,revealVotes:!!cfg.revealVotes,tieMode:cfg.tieMode==='revote'?'revote':'none',nightSec:n(cfg.nightSec,30,10,120),discussionSec:n(cfg.discussionSec,90,15,300),voteSec:n(cfg.voteSec,30,10,120)};
  const roleTotal=c.mafiaCount+c.detectiveCount+c.doctorCount+c.jesterCount+c.guardCount+c.poisonerCount+c.informantCount+c.godfatherCount;if(roleTotal>c.maxPlayers)return s.emit('errorMsg','عدد الأدوار أكبر من عدد اللاعبين');if(c.godfatherCount>0&&c.mafiaCount<1)return s.emit('errorMsg','العرّاب يحتاج إلى مافيا');if(g.players.length>c.maxPlayers)return s.emit('errorMsg','عدد اللاعبين الحالي أكبر من العدد المحدد');g.config=c;broadcastMafia(g);
 });
 s.on('mafia:start',()=>{
  const g=games.get(s.data?.game);if(!g||g.type!=='mafia'||g.phase!=='lobby'||g.hostId!==s.data?.pid)return;const c=g.config;if(g.players.length!==c.maxPlayers)return s.emit('errorMsg','يجب اكتمال عدد اللاعبين أولًا');
  const roleTotal=c.mafiaCount+c.detectiveCount+c.doctorCount+c.jesterCount+c.guardCount+c.poisonerCount+c.informantCount+c.godfatherCount;if(roleTotal>g.players.length)return s.emit('errorMsg','عدد الأدوار أكبر من عدد اللاعبين');let pool=[...g.players];for(let i=pool.length-1;i>0;i--){const j=crypto.randomInt(i+1);[pool[i],pool[j]]=[pool[j],pool[i]]}let k=0;pool.forEach(p=>{p.alive=true;p.role='citizen'});const assign=(r,n)=>{for(let i=0;i<n;i++)pool[k++].role=r};assign('mafia',c.mafiaCount);assign('godfather',c.godfatherCount);assign('detective',c.detectiveCount);assign('doctor',c.doctorCount);assign('guard',c.guardCount);assign('poisoner',c.poisonerCount);assign('informant',c.informantCount);assign('jester',c.jesterCount);g.rolesAssigned=true;g.phase='role';g.phaseUntil=Date.now()+17000;g.nightActions={};g.mafiaSelections={};g.votes={};g.investigationResults={};g.poisoned={};g.voteRequests={};g.voteResult=null;g.mafiaConsensus=null;g.round=0;g.lastNightResult=null;g.winnerText=null;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='role'&&g.phaseUntil===until)mafiaNight(g)},17100);
 });
 s.on('mafia:requestVote',()=>{const g=games.get(s.data?.game),p=g?.players.find(x=>x.id===s.data?.pid);if(!g||g.type!=='mafia'||g.phase!=='discussion'||!p||p.alive===false||p.role==='jester')return;g.voteRequests=g.voteRequests||{};g.voteRequests[p.id]=true;const eligible=g.players.filter(x=>x.alive!==false&&x.role!=='jester');g.earlyVoteCount=Object.keys(g.voteRequests).filter(id=>eligible.some(x=>x.id===id)).length;g.earlyVoteTotal=eligible.length;broadcastMafia(g);if(g.earlyVoteCount>eligible.length/2)mafiaStartVote(g)});
 s.on('mafia:nightAction',({targetId,confirm=false}={})=>{
  const g=games.get(s.data?.game),p=g?.players.find(x=>x.id===s.data?.pid);if(!g||g.type!=='mafia'||g.phase!=='night'||!p||p.alive===false||Date.now()>g.phaseUntil)return;if(['citizen','jester'].includes(p.role))return;
  const target=g.players.find(x=>x.id===targetId&&x.alive!==false&&x.id!==p.id);if(!target)return;g.nightActions=g.nightActions||{};if(g.nightActions[p.id]?.submitted)return;
  if(['mafia','godfather'].includes(p.role)){g.mafiaSelections=g.mafiaSelections||{};g.mafiaSelections[p.id]=target.id;g.nightActions[p.id]={submitted:!!confirm,targetId:target.id};const ms=g.players.filter(x=>x.alive!==false&&['mafia','godfather'].includes(x.role));const picks=ms.map(x=>g.nightActions[x.id]?.submitted?g.nightActions[x.id].targetId:null).filter(Boolean);g.mafiaConsensus={confirmed:ms.filter(x=>g.nightActions[x.id]?.submitted).length,total:ms.length,agreedTarget:picks.length===ms.length&&picks.every(x=>x===picks[0])?picks[0]:null};
  }else if(confirm){g.nightActions[p.id]={submitted:true,targetId:target.id};if(['detective','informant'].includes(p.role))g.investigationResults[p.id]=['mafia','godfather'].includes(target.role)?'mafia':'not_mafia';else if(p.role==='poisoner'){g.poisoned=g.poisoned||{};g.poisoned[target.id]={by:p.id,round:g.round||0};}}else g.nightActions[p.id]={submitted:false,targetId:target.id};broadcastMafia(g);
 });
 ...
 s.on('mafia:vote',({targetId}={})=>{const g=games.get(s.data?.game),p=g?.players.find(x=>x.id===s.data?.pid);if(!g||g.type!=='mafia'||g.phase!=='vote'||!p||p.alive===false||p.role==='jester'||g.votes[p.id])return;const target=g.players.find(x=>x.id===targetId&&x.alive!==false&&x.id!==p.id);if(!target)return;g.votes[p.id]=target.id;broadcastMafia(g);const eligible=g.players.filter(x=>x.alive!==false&&x.role!=='jester');if(eligible.every(x=>g.votes[x.id]))mafiaResolveVote(g)});
 s.on('mafia:restart',()=>{const g=games.get(s.data?.game);if(!g||g.type!=='mafia'||g.hostId!==s.data?.pid||g.phase!=='finished')return;g.players.forEach(p=>{p.alive=true;p.role=undefined});g.rolesAssigned=false;g.phase='lobby';g.phaseUntil=0;g.nightActions={};g.mafiaSelections={};g.votes={};g.voteRequests={};g.earlyVoteCount=0;g.earlyVoteTotal=0;g.investigationResults={};g.poisoned={};g.lastNightResult=null;g.winnerText=null;g.voteResult=null;g.mafiaConsensus=null;broadcastMafia(g)});
 function mafiaNight(g){if(mafiaCheckWin(g))return;g.round=(g.round||0)+1;g.phase='night';g.phaseUntil=Date.now()+g.config.nightSec*1000;g.nightActions={};g.mafiaSelections={};g.votes={};g.voteRequests={};g.earlyVoteCount=0;g.earlyVoteTotal=0;g.investigationResults={};g.mafiaConsensus=null;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='night'&&g.phaseUntil===until)mafiaResolveNight(g)},g.config.nightSec*1000+100)}
 function mafiaResolveNight(g){const alive=g.players.filter(p=>p.alive!==false),actions=g.nightActions||{},mafiaPlayers=alive.filter(p=>['mafia','godfather'].includes(p.role));const confirmed=mafiaPlayers.map(p=>actions[p.id]?.submitted?actions[p.id].targetId:null).filter(Boolean),tally={};confirmed.forEach(id=>tally[id]=(tally[id]||0)+1);const max=Math.max(0,...Object.values(tally)),top=Object.keys(tally).filter(id=>tally[id]===max);let victim=null;if(top.length===1&&max>0)victim=g.players.find(p=>p.id===top[0]&&p.alive!==false);const protects=alive.filter(p=>['doctor','guard'].includes(p.role)).map(p=>actions[p.id]?.submitted?actions[p.id].targetId:null).filter(Boolean);let saved=!!victim&&protects.includes(victim.id);if(victim&&!saved){victim.alive=false;g.lastNightResult={victimId:victim.id,victimName:victim.name,saved:false,poisoned:false}}else if(victim&&saved)g.lastNightResult={victimId:null,victimName:null,saved:true,poisoned:false};else g.lastNightResult={victimId:null,victimName:null,saved:false,poisoned:false};const poisonTargets=Object.entries(g.poisoned||{}).filter(([id,v])=>{const p=g.players.find(x=>x.id===id);return p&&p.alive!==false&&Number(v?.round)<(g.round||0)}).map(([id])=>id);for(const id of poisonTargets){const p=g.players.find(x=>x.id===id);if(p&&!protects.includes(id)){p.alive=false;if(!g.lastNightResult?.victimId)g.lastNightResult={victimId:p.id,victimName:p.name,saved:false,poisoned:true}}}g.mafiaSelections={};g.mafiaConsensus=null;if(mafiaCheckWin(g))return;g.phase='discussion';g.phaseUntil=Date.now()+g.config.discussionSec*1000;g.voteRequests={};g.earlyVoteCount=0;g.earlyVoteTotal=g.players.filter(p=>p.alive!==false&&p.role!=='jester').length;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='discussion'&&g.phaseUntil===until)mafiaStartVote(g)},g.config.discussionSec*1000+100)}
 function mafiaStartVote(g){g.phase='vote';g.phaseUntil=Date.now()+g.config.voteSec*1000;g.votes={};g.voteRequests={};g.earlyVoteCount=0;g.voteResult=null;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='vote'&&g.phaseUntil===until)mafiaResolveVote(g)},g.config.voteSec*1000+100)}
 function mafiaResolveVote(g){if(g.phase!=='vote')return;const counts={};Object.values(g.votes||{}).forEach(id=>counts[id]=(counts[id]||0)+1);const max=Math.max(0,...Object.values(counts)),top=Object.keys(counts).filter(id=>counts[id]===max),rows=g.players.filter(p=>p.alive!==false).map(p=>({name:p.name,id:p.id,count:counts[p.id]||0})).sort((a,b)=>b.count-a.count);g.voteResult={rows,tie:!max||top.length!==1,eliminatedId:null,eliminatedName:null,eliminatedRole:null};if(!max||top.length!==1){g.phase='voteResult';g.phaseUntil=Date.now()+6200;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='voteResult'&&g.phaseUntil===until){if(g.config.tieMode==='revote'){g.voteResult=null;mafiaStartVote(g)}else{g.voteResult=null;mafiaNight(g)}}},6300);return}const victim=g.players.find(p=>p.id===top[0]&&p.alive!==false);if(victim){victim.alive=false;if(g.poisoned)delete g.poisoned[victim.id];g.voteResult.eliminatedId=victim.id;g.voteResult.eliminatedName=victim.name;g.voteResult.eliminatedRole=g.config.revealRole?victim.role:undefined;if(victim.role==='jester'){g.winnerText='jester';g.phase='voteResult';g.phaseUntil=Date.now()+6200;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='voteResult'&&g.phaseUntil===until){g.voteResult=null;g.phase='finished';g.finishedAt=Date.now();broadcastMafia(g)}},6300);return}}if(mafiaCheckWin(g))return;g.phase='voteResult';g.phaseUntil=Date.now()+6200;broadcastMafia(g);const until=g.phaseUntil;setTimeout(()=>{if(g.phase==='voteResult'&&g.phaseUntil===until){g.voteResult=null;mafiaNight(g)}},6300)}
 function mafiaCheckWin(g){const alive=g.players.filter(p=>p.alive!==false),m=alive.filter(p=>['mafia','godfather'].includes(p.role)).length,other=alive.length-m;if(m===0){g.phase='finished';g.winnerText='citizens';g.finishedAt=Date.now();broadcastMafia(g);return true}if(m>=other){g.phase='finished';g.winnerText='mafia';g.finishedAt=Date.now();broadcastMafia(g);return true}return false}
